...
imgD = imresize(imgA,[hh ww]);
imgD = uint8(255*(imgD -min(min(imgD )))/(max(max(imgD))-min(min(imgD))));
%*******************
%*****Output********
%*******************

imwrite(imgC,strcat(strFolder,'output\resultC.jpg'));
imwrite(imgD,strcat(strFolder,'output\resultD.jpg'));
